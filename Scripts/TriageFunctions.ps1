function Get-EntraApplicationsForSpecificUsers {
    ...
    foreach ($user in $validUsers) {
        ...
        try {
            $ownedApps = [System.Collections.Generic.List[object]]::new()
            foreach ($obj in Get-MgUserOwnedObject -UserId $user.Id -All) {
                if ($obj.AdditionalProperties.'@odata.type' -eq '#microsoft.graph.application') {
                    $ownedApps.Add($obj)
                }
            }
                        ...
                        $appObject = [PSCustomObject]@{
                            ...
                            RequiredApiPermissionCount = if ($app.RequiredResourceAccess) {
                                ...
                                foreach ($resourceAccess in $app.RequiredResourceAccess) {
                                    $total += $resourceAccess.ResourceAccess.Count
                                }
                                $total
                            } else { 0 }
...
#>
    ...
    foreach ($operation in $Operations) {
        ...
        try {
            ...
            if ($results) {
                $processedResults = [System.Collections.Generic.List[object]]::new($results.Count)
                foreach ($result in $results) {
                    $record = $result | Select-Object *
                    if ($record.AuditData) {
                        try {
                    ...
                    $processedResults.Add($record)
                }

                $allResults.AddRange($processedResults)
                ...
                $csvResults = [System.Collections.Generic.List[object]]::new($processedResults.Count)
                foreach ($processedResult in $processedResults) {
                    $flatRecord = $processedResult | Select-Object * -ExcludeProperty AuditData

                    # Add key AuditData fields as separate columns
                    if ($processedResult.AuditData) {
                        $flatRecord | Add-Member -NotePropertyName "AuditData_UserId" -NotePropertyValue $processedResult.AuditData.UserId -Force
                        $flatRecord | Add-Member -NotePropertyName "AuditData_ClientIP" -NotePropertyValue $processedResult.AuditData.ClientIP -Force
                        $flatRecord | Add-Member -NotePropertyName "AuditData_UserAgent" -NotePropertyValue $processedResult.AuditData.UserAgent -Force
                        $flatRecord | Add-Member -NotePropertyName "AuditData_ObjectId" -NotePropertyValue $processedResult.AuditData.ObjectId -Force

                        $flatRecord | Add-Member -NotePropertyName "AuditData_Raw" -NotePropertyValue ($processedResult.AuditData | ConvertTo-Json -Compress -Depth 5) -Force
                    }
                    $csvResults.Add($flatRecord)
                }
    ...
    if ( $allResults.Count -gt 0) {
        ...
        switch ($Output) {
            "CSV" {
                ...
                $combinedCsvResults = [System.Collections.Generic.List[object]]::new($allResults.Count)
                foreach ($allResult in $allResults) {
                    $flatRecord = $allResult | Select-Object * -ExcludeProperty AuditData

                    if ($allResult.AuditData) {
                        $flatRecord | Add-Member -NotePropertyName "AuditData_UserId" -NotePropertyValue $allResult.AuditData.UserId -Force
                        $flatRecord | Add-Member -NotePropertyName "AuditData_ClientIP" -NotePropertyValue $allResult.AuditData.ClientIP -Force
                        $flatRecord | Add-Member -NotePropertyName "AuditData_UserAgent" -NotePropertyValue $allResult.AuditData.UserAgent -Force
                        $flatRecord | Add-Member -NotePropertyName "AuditData_ObjectId" -NotePropertyValue $allResult.AuditData.ObjectId -Force
                        $flatRecord | Add-Member -NotePropertyName "AuditData_Raw" -NotePropertyValue ($allResult.AuditData | ConvertTo-Json -Compress -Depth 5) -Force
                    }
                    $combinedCsvResults.Add($flatRecord)
                }
