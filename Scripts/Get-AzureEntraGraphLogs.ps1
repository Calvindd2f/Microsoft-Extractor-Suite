...
#>
	...
	begin {
		...
		$eventTypesToProcess = [System.Collections.Generic.List[string]]::new()
		if ($EventTypes -contains 'All') {
			if ($UserIds -and $UserIds.Count -gt 0) {
				$eventTypesToProcess.Add('interactiveUserAndNonInteractiveUser')
				...
				$eventTypesToProcess.AddRange(@('interactiveUserAndNonInteractiveUser', 'servicePrincipal', 'managedIdentity'))
			}
		} elseif ($EventTypes -contains 'interactiveUser' -and $EventTypes -contains 'nonInteractiveUser') {
			foreach ($type in $EventTypes) {
				if ($type -ne 'interactiveUser' -and $type -ne 'nonInteractiveUser') {
					$eventTypesToProcess.Add($type)
				}
			}
			$eventTypesToProcess.Insert(0, 'interactiveUserAndNonInteractiveUser')
		}
		else {
		...
		foreach ($eventType in $eventTypesToProcess) {
			...
			if (!(Test-Path $eventTypeDir)) {
				$null = New-Item -ItemType Directory -Force -Path $eventTypeDir
			}

			$filterQuery = "createdDateTime ge $StartDate and createdDateTime le $EndDate"

			if ($UserIds -and $UserIds.Count -gt 0) {
				$userFilters = foreach ($userId in $UserIds) { "startsWith(userPrincipalName, '$userId')" }
			...
			try {
				Do {
					...
					if ($responseJson.value) {
						...
						$sw = [System.IO.StreamWriter]::new($filePath, $true, [System.Text.Encoding]::GetEncoding($Encoding))
						try {
							if ($Output -eq "JSON" ) {
						...
						# Instead of pipeline with ForEach-Object, use direct .NET parsing and LINQ for sorting
						$dates = [System.Collections.Generic.List[DateTime]]::new($currentBatchCount)
						foreach ($item in $responseJson.value) {
							$dates.Add([DateTime]::Parse($item.CreatedDateTime, [System.Globalization.CultureInfo]::InvariantCulture))
						}
						$dates.Sort()
	...
	process {
		...
		# Calculate final statistics and display summary
...
function Get-GraphEntraAuditLogs {
	...
	begin {
		...
		if ($UserIds -and $UserIds.Count -gt 0) {
			$userFilters = foreach ($userId in $UserIds) { "startsWith(initiatedBy/user/userPrincipalName, '$userId')" }
			$filterQuery += " and (" + ($userFilters -join " or ") + ")"

			if ($All.IsPresent) {
				$targetFilters = foreach ($userId in $UserIds) { "targetResources/any(tr: tr/userPrincipalName eq '$userId')" }
		...
		try {
			Do {
				...
				if ($responseJson.value) {
					...
					$dates = [System.Collections.Generic.List[DateTime]]::new($currentBatchCount)
					foreach ($item in $responseJson.value) {
						$dates.Add([DateTime]::Parse($item.activityDateTime, [System.Globalization.CultureInfo]::InvariantCulture))
					}
					$dates.Sort()

					$from = $dates[0]
					$fromstr = $from.ToString('yyyy-MM-ddTHH:mmZ')
					$to = $dates[$dates.Count - 1].ToString('yyyy-MM-ddTHH:mmZ')
