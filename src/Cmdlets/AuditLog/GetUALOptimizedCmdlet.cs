...
namespace Microsoft.ExtractorSuite.Cmdlets.AuditLog
{
    ...
    public class GetUALOptimizedCmdlet : BaseCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            {
                Logger?.LogError($"Error processing audit logs: {ex.Message}", ex);
        ...
        private async Task<int> GetRecordCountAsync(string recordType)
        {
            ...
            {
                Logger?.LogWarning($"Failed to get record count: {ex.Message}");
            ...
            while (!success && retryCount < MaxRetries)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    if (retryCount >= MaxRetries)
                    {
                        ...
                        Logger?.LogError($"Window {window.Id} failed after {MaxRetries} retries: {ex.Message}", ex);
                        WriteWarning($"Failed to process window {window.Id}: {ex.Message}");
                    }
                    else
                    {
                        var delay = Math.Pow(2, retryCount) * 1000;
                        Logger?.LogWarning($"Window {window.Id} failed (attempt {retryCount}), retrying in {delay}ms: {ex.Message}");
