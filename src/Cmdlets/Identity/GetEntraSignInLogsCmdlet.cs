...
namespace Microsoft.ExtractorSuite.Cmdlets.Identity
{
    ...
    public class GetEntraSignInLogsCmdlet : BaseCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            {
                Logger?.LogError($"Error processing sign-in logs: {ex.Message}", ex);
        ...
        private async Task ProcessSignInLogsAsync(string filter)
        {
            ...
            {
                Logger?.LogError($"Graph API error: {ex.Message}", ex);
        ...
        private void ProcessSignInLog(SignInLog log)
        {
            ...
            {
                _stats.CountryCount[log.Location.CountryOrRegion] = 
                    _stats.CountryCount.GetValueOrDefault(log.Location.CountryOrRegion, 0) + 1;
            }
        }
        ...
        private void CheckForSuspiciousActivity(SignInLog log)
        {
            ...
            if (log.RiskEventTypesV2?.Any(e => 
                e.Contains("unfamiliar", StringComparison.OrdinalIgnoreCase) ||
                e.Contains("anonymous", StringComparison.OrdinalIgnoreCase) ||
                e.Contains("malware", StringComparison.OrdinalIgnoreCase) ||
                e.Contains("impossible", StringComparison.OrdinalIgnoreCase)) == true)
            {
                suspiciousReasons.Add($"Suspicious risk events: {string.Join(", ", log.RiskEventTypesV2)}");
            }
            
            // Check for legacy authentication
            if (log.ClientAppUsed?.Contains("legacy", StringComparison.OrdinalIgnoreCase) == true)
            {
